...
async def test_validate_payload_valid(monkeypatch):
    ...
    fake_event_checker = MagicMock()
    validator = BittensorValidationMechanism(fake_repository, fake_event_checker)
...
async def test_verify_block_ranges_valid(monkeypatch):
    ...
    validator = BittensorValidationMechanism(event_store_repository=None, event_checker=None)
...
async def test_verify_edge_data_success(monkeypatch):
    ...
    mock_event_checker = MagicMock()
    # Return empty list to indicate all events were found
    mock_event_checker.check_events_by_hash = AsyncMock(return_value=[])

    # Create the validator with our mocks
    validator = BittensorValidationMechanism(mock_event_store_repository, mock_event_checker)
    ...
    mock_event_checker.check_events_by_hash.assert_called_once()
    
    # Optional: verify the converted event data format
    called_args = mock_event_checker.check_events_by_hash.call_args[0][0]
    ...
    assert called_args[0]["rao_amount"] == 100